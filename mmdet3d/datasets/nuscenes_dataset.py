...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(self,
        ...
        # self.mask_nuscenes = self.mask_loader(self.data_root)
    ...
    def get_data_info(self, index):
        ...
        ego2lidar = transform_matrix(translation=lidar2ego_translation, rotation=Quaternion(lidar2ego_rotation),
                                     inverse=True)
        input_dict.update(dict(lidar2ego=lidar2ego))
        input_dict.update(dict(ego2lidar=ego2lidar))
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                for cam_type, cam_info in info['cams'].items():
                    ...
                    lidar2img_rt = (viewpad @ lidar2cam_rt.T)
                    lidar2img_rts.append(lidar2img_rt)
                    # cam_mask.append(self.mask_nuscenes[cam_type])
                input_dict.update(
                    dict(
                        img_filename=image_paths,
                        lidar2img=lidar2img_rts,
    ...
    def mask_loader(self, root):
        mask_path = osp.join(root, 'nuscenes_mask')
        mask = {}
        for cam in ['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_BACK_LEFT',
        'CAM_BACK', 'CAM_BACK_RIGHT']:
            cam_mask_path = osp.join(mask_path, cam+'_mask.png')
            with open(cam_mask_path,'rb') as f:
                with Image.open(f) as img:
                    mask[cam] = img.convert('L')
        return mask
