...
from .ray import generate_dense_rays
...
class NuScenesDatasetOccpancyNerfDense(NuScenesDatasetOccpancy):
    def __init__(self, 
                use_rays=False,
    ...
    def get_dense_rays(self, index):
        ...
        rays = generate_dense_rays(c2w=sensor2egos,intrins=intrins,render_downsample=self.render_downsample)
        return rays
    ...
    def get_data_info(self, index):
        input_dict = super(NuScenesDatasetOccpancyNerfDense, self).get_data_info(index)
        ...
        if self.use_rays:
            rays_info = self.get_dense_rays(index)
            input_dict['rays'] = rays_info
        else:
            input_dict['rays'] = torch.zeros((1))
        input_dict['label_depth'] = self.get_depth_gt(index)
        return input_dict
    
    def get_depth_gt(self, index):
        gt_depths = []
        info = self.data_infos[index]
        for cam_name in info['cams'].keys():
            img_file_path = info['cams'][cam_name]['data_path']
            coor, label_depth = load_depth(img_file_path, self.depth_gt_path)
            gt_depth = torch.zeros(900//self.render_downsample,1600//self.render_downsample)
            coor=torch.Tensor(coor/self.render_downsample).long()
            label_depth = torch.Tensor(label_depth)
            gt_depth[coor[:,1],coor[:,0]] = label_depth
            gt_depths.append(gt_depth)
        return torch.stack(gt_depths)