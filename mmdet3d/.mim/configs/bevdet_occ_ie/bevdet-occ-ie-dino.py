...
train_pipeline = [
    ...
    dict(
        type='Collect3D', keys=['img_inputs', 'gt_depth', 'voxel_semantics', 'gt_bboxes_3d', 'gt_labels_3d',
                                'mask_lidar','mask_camera','logits'])
]

test_pipeline = [
    dict(type='PrepareImageInputs', data_config=data_config, sequential=True, load_semantics=True),
    dict(
        type='LoadAnnotationsBEVDepth',
        bda_aug_conf=bda_aug_conf,
        classes=class_names,
