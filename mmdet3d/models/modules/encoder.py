...
class OccEncoder(TransformerLayerSequence):
    ...
    def point_sampling(self, reference_points, pc_range, post_rots, post_trans, img_metas):
        ...
        post_trans = post_trans.reshape(D, B, num_cam, 1, 2).repeat(1,1,1,num_query,1)
        post_rots = post_rots.reshape(D, B, num_cam, 1, 2, 2).repeat(1,1,1,num_query,1,1)
        reference_points_cam = post_rots.matmul(reference_points_cam.unsqueeze(-1)).squeeze(-1)
    ...
    def forward(self,
        ...
        ref_3d = torch.matmul(bda.reshape(-1,1,1,3,3),ref_3d.unsqueeze(-1)).squeeze(-1)

        reference_points_cam, volume_mask = self.point_sampling(
            ref_3d, self.pc_range, post_rots, post_trans, kwargs['img_metas'])
...
@TRANSFORMER_LAYER_SEQUENCE.register_module()
class OccEncoderPrompt(OccEncoder):
    def __init__(self, *args, pc_range=None, return_intermediate=False, **kwargs):
        super(OccEncoderPrompt, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate

        self.pc_range = pc_range
        self.fp16_enabled = False
        self.grid_lower_bound = torch.Tensor(pc_range[:3])
        self.grid_interval = torch.Tensor([0.4, 0.4, 0.4])
        self.cls_map = {5: 1, 7: 2, 3: 3, 0: 4, 2: 5, 6: 6, 8: 7, 9: 8, 4: 9, 1: 10}
        self.class_num = 18
        # self.prompt_projector = nn.Sequential(
        #     nn.Linear(in_features=128+18,
        #               out_features=128),
        #     # nn.ReLU()
        # )
    @auto_fp16()
    def forward(self,
        ...
        prompt_shape = [volume_query.size(0), volume_h, volume_w, volume_z, self.class_num]
        if volume_query.requires_grad == False:
            # Test phase shouldn't include gt
            prompt = torch.zeros(prompt_shape).cuda()
            # prompt = self.get_prompt(prompt_shape, img_metas, type='gt')
        else:
            prompt = self.get_prompt(prompt_shape, img_metas, type='gt')
        prompt = prompt.reshape(volume_query.size(0), -1, self.class_num)
        # volume_query = torch.cat([volume_query,prompt], dim=2)
        volume_query = self.fuse_prompt(volume_query, prompt)
    ...
    def get_prompt(self, prompt_shape, img_metas, type='gt'):
        prompt = torch.zeros(prompt_shape).cuda() # (bs, volume_h, volume_w, volume_z, cls_num)
        ...
        for batch_id, img_meta in enumerate(img_metas):
            ...
            coord = ((coord - self.grid_lower_bound.to(coord)) /
                self.grid_interval.to(coord)).int()
            # filter outer bound
            coord[coord<0]=0
            coord[coord>200]=200
            coord[...,2][coord[...,2]>16]=16
            # prompt 100*100*8 ,coord in 200*200*16
            coord = torch.div(coord,2,rounding_mode='trunc').long()
            for vertex, label in zip(coord, labels):
                if vertex.dim()==1:
                    print("non")
                self.axis_align_pooling(prompt,vertex,label,batch_id)

        return prompt
    
    def axis_align_pooling(self, grid, vertex, target, batch_id):
        ...
        grid[batch_id,slice(*x_range), slice(*y_range), slice(*z_range), self.cls_map[target.item()]] = 1

    def fuse_prompt(self, query, prompt):
        volume_query = torch.cat([query, prompt], dim=2)
        # volume_query = self.prompt_projector(volume_query)
        return volume_query

@TRANSFORMER_LAYER_SEQUENCE.register_module()
class OccEncoderSemBEV(OccEncoder):
    def __init__(self, *args, pc_range=None, return_intermediate=False, **kwargs):
        super(OccEncoderSemBEV, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
    ...
    def forward(self,
        ...
        prompt = self.get_prompt(volume_h, volume_w, volume_z, img_metas)
        # if volume_query.requires_grad == False:
        #     # Test phase shouldn't include gt
        #     prompt = torch.zeros(prompt_shape).cuda()
        #     # prompt = self.get_prompt(prompt_shape, img_metas, type='gt')
    ...
    def get_prompt(self, vh, vw, vz, img_metas):
        prompt = []
        for batch_id, img_meta in enumerate(img_metas):
            sem_bev = img_meta.get('sem_bev')
            sem_bev = sem_bev.permute(0,3,2,1).unsqueeze(0)
            sem_bev = F.interpolate(sem_bev,(vh, vw, vz)).flatten(2)
            sem_bev = sem_bev.permute(0,2,1)
            prompt.append(sem_bev)
