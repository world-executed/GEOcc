...
class VFNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.height=height
        self.width=width
        self.aug_depth=False

        self.create_grids_flag = False
    ...
    def create_grids(self):
        if self.create_grids_flag:
            return
        self.create_grids_flag = True
        self.batch_size = 1
    ...
    def expand_grids(self):
        if self.voxel_pts.shape[0]==self.batch_size:
            return
        elif self.voxel_pts.shape[0]<=self.batch_size:
            self.voxel_pts = self.voxel_pts.repeat(self.batch_size, 1, 1)
            self.pixel_grid = self.pixel_grid.repeat(self.batch_size, 1, 1)
            self.pixel_ones = self.pixel_ones.repeat(self.batch_size, 1, 1, 1)
            self.depth_grid = self.depth_grid.repeat(self.batch_size, 1, 1, 1)
        elif self.voxel_pts.shape[0]>=self.batch_size:
            self.voxel_pts = self.voxel_pts[0].unsqueeze(0)
            self.pixel_grid = self.pixel_grid[0].unsqueeze(0)
            self.pixel_ones = self.pixel_ones[0].unsqueeze(0)
            self.depth_grid = self.depth_grid[0].unsqueeze(0)
    ...
    def backproject_into_voxel(self, feats_agg, input_mask, intrinsics, extrinsics_inv, post_rots, post_trans, scale):
        """
        ...
        for cam in range(self.num_cams):
            ...
            post_rots_mat = post_rots[:,cam, :2, :2]
            post_trans_mat = post_trans[:,cam, :2]
            pix_coords = self.calculate_sample_pixel_coords(K_mat, v_pts_local, w_dim, h_dim, post_rots_mat, post_trans_mat, scale)
    ...
    def calculate_sample_pixel_coords(self, K, v_pts, w_dim, h_dim, post_rots, post_trans, scale):
        """
        ...
        if not torch.all(torch.isfinite(pix_coords)):
            pix_coords = torch.clamp(pix_coords, min=-w_dim*2*scale, max=w_dim*2*scale)
        ...
        pix_coords[:, :, :, 0] = pix_coords[:, :, :, 0] / (w_dim * scale - 1)
        pix_coords[:, :, :, 1] = pix_coords[:, :, :, 1] / (h_dim * scale - 1)
    ...
    def project_voxel_into_image(self, voxel_feat, inv_K, extrinsics, post_rots, post_trans, scale):
        """
        ...
        for cam in range(self.num_cams):
            ...
            pixel_grid = pixel_grid - post_trans[:,cam,:].reshape(self.batch_size, 3, 1)
    ...
    def forward(self, inputs, feats_agg):
        ...
        post_rots = inputs['post_rots']
        post_trans = inputs['post_trans']
        scale = inputs['scale']
        ...
        self.create_grids()
        self.batch_size = feats_agg.shape[0]
        self.expand_grids()
        ...
        voxel_feat = self.backproject_into_voxel(feats_agg, mask, K, extrinsics_inv, post_rots, post_trans, scale)
            
        if self.model == 'depth':
            # for each pixel, collect voxel features -> output image feature     
            proj_feats = self.project_voxel_into_image(voxel_feat, inv_K, extrinsics, post_rots, post_trans, scale)
