...
from mmdet3d.utils.semkitti import semantic_kitti_class_frequencies
from mmdet3d.utils.nusc import nusc_class_frequencies
import pdb
...
class Mask2FormerOccHead(MaskFormerHead):
    ...
    def __init__(self,
        ...
        self.test_cfg = test_cfg.pts
        self.train_cfg = train_cfg.pts
        if train_cfg:
        ...
        class_weights = 1 / np.log(nusc_class_frequencies[:-1])
    ...
    def _get_target_single(self, cls_score, mask_pred, gt_labels, gt_masks, img_metas):
        ...
        # label target
        labels = gt_labels.new_full((self.num_queries, ), self.num_classes - 1, dtype=torch.long)
    ...
    def preprocess_gt(self, gt_occ, img_metas, mask_camera):
        ...
        targets = multi_apply(preprocess_occupancy_gt, gt_occ, num_class_list, img_metas, mask_camera)
    ...
    def forward_train(self,
        ...
        gt_labels, gt_masks = self.preprocess_gt(gt_occ, img_metas, mask_camera)
    ...
    def format_results(self, mask_cls_results, mask_pred_results):
        ...
        """这里把最后的第17类给去掉了，不应该去掉free类别"""
        mask_cls = F.softmax(mask_cls_results, dim=-1)
...
@HEADS.register_module()
class Mask2FormerOccHeadPrompt(Mask2FormerOccHead):
    def __init__(self, **kwargs):
        super(Mask2FormerOccHead, self).__init__( **kwargs)
    ...
    def forward(self, 
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            decoder_semantic_embeddings = None
            decoder_positional_encoding = decoder_positional_encoding + decoder_semantic_embeddings
