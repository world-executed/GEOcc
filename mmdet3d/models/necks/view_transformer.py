...
class LSSViewTransformer(BaseModule):
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        # debug (1,32,32,256,256) 突然变成了（1，1024，256，256）？？
        # if len(bev_feat.shape) != 5:
        #     print("warning!!",bev_feat.shape)
        #     B, CZ, Y, X = bev_feat.shape
        #     bev_feat = bev_feat.reshape(B,32,CZ//32,Y,X)
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def get_mlp_input(self, sensor2ego, ego2global, intrin, post_rot, post_tran, bda):
        B, N, _, _ = sensor2ego.shape
        bda_shape = bda.shape[-2:]
        bda = bda.view(B, 1, bda_shape[0], bda_shape[1]).repeat(1, N, 1, 1)
...
@NECKS.register_module()
class LSSViewTransformerBEVStereoPreDepth(LSSViewTransformerBEVStereo):

    def __init__(self,  **kwargs):
        super(LSSViewTransformerBEVStereoPreDepth, self).__init__(**kwargs)
    
    def forward(self, input, stereo_metas=None, pre_depth=None):
        (x, rots, trans, intrins, post_rots, post_trans, bda,
         mlp_input) = input[:8]
        ...
        if pre_depth is not None:
            pre_depth = torch.cat(pre_depth.unbind(0),dim=0)
            pre_depth = pre_depth.softmax(dim=1)
            depth_digit = depth_digit.softmax(dim=1)
            depth_digit = (depth_digit + pre_depth)/2
        depth = depth_digit.softmax(dim=1)
        bev_feat, depth = self.view_transform(input, depth, tran_feat)
        return bev_feat, depth

@NECKS.register_module()
class LSSViewTransformerBEVSem(LSSViewTransformerBEVStereo):
    def __init__(self, transformer_cfg = None, **kwargs):
        super().__init__(**kwargs)
        # self.up = nn.Upsample(scale_factor=self.downsample,mode='bilinear')
        # self.down = nn.Upsample(scale_factor=1/self.downsample,mode='bilinear')
        # self.down = nn.Upsample(scale_factor=1/self.downsample,mode='nearest')
        ...
        #     padding=1,
        #     bias=True)
        # self.PromptTransformer = build_transformer_layer_sequence(transformer_cfg)
        # self.pos_embed = nn.Parameter(torch.zeros(1, 16*44, 32).cuda())
        # nn.init.trunc_normal_(self.pos_embed, std=0.02)

    def forward(self, input, stereo_metas=None):
        (x, rots, trans, intrins, post_rots, post_trans, bda,
         mlp_input) = input[:8]
        logits = stereo_metas.get('logits',None)
        B, N, C, H, W = x.shape
        x = x.view(B * N, C, H, W)
        x = self.depth_net(x, mlp_input, stereo_metas)
        depth_digit = x[:, :self.D, ...]
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]
        depth = depth_digit.softmax(dim=1)
        # group_feat = self.group(tran_feat, logits)
        # sem_tran_feat = self.prompt_transformer_encoder(tran_feat, logits)
        bev_feat, depth = self.view_transform(input, depth, tran_feat)
        return bev_feat, depth

    # def group(self, tran_feat, logits):
    #     if logits is None:
    #         return tran_feat
    #     # upsample tran_feat to logits shape
    #     up_tran_feat = self.up(tran_feat)
    #     up_tran_feat = up_tran_feat.permute(0,2,3,1)
    #     B,N,_,H,W = logits.shape
    #     logits = logits.reshape(B*N,H,W)
    #     idx_list = torch.unique(logits)
    #     for idx in idx_list:
    #         mask = (logits == idx)
    #         up_tran_feat[mask] = up_tran_feat[mask].mean(dim=0)
    #     group_feat = self.down(up_tran_feat.permute(0,3,1,2))
    #     return group_feat
    
    def group(self, tran_feat, logits):
        """
        tran_feat: shape(BN, C, H, W)
        """
        if logits is None:
            return tran_feat
        tran_feat = tran_feat.clone() # 加了一个clone解决了RuntimeError: one of the variables needed for gradient computation has been modified by an inplace operation的问题
        group_feat = tran_feat.permute(0,2,3,1) # group_feat shape(BN, H, W, C)
        B,N,C,H,W = logits.shape
        logits = logits.reshape(B*N,C,H,W)
        logits = self.down(logits).squeeze(dim=1)
        new_group_feat = []
        for batch in range(B*N):
            group_feat_batch = group_feat[batch]
            logits_batch = logits[batch]
            idx_list = torch.unique(logits_batch)
            for idx in idx_list:
                mask = (logits_batch == idx)
                group_feat_batch[mask] = group_feat_batch[mask].mean(dim=0)
            new_group_feat.append(group_feat_batch.permute(2,0,1))
        new_group_feat = torch.stack(new_group_feat)
        feat_fusion = torch.cat([tran_feat, new_group_feat], dim=1)
        feat_fusion = self.feat_fusion(feat_fusion)
        return feat_fusion
    
    def prompt_transformer_encoder(self, tran_feat, logits):
        if logits is None:
            return tran_feat
        tran_feat = tran_feat.clone() 
        query = tran_feat.permute(0,2,3,1) # query shape(BN, H, W, C)
        B,N,C,H,W = logits.shape
        logits = logits.reshape(B*N,C,H,W)
        logits = self.down(logits).permute(0, 2, 3, 1) # shape (BN, H, W, 1)
        
        bs, dh, dw, c = query.shape
        query = query.reshape(bs, dh*dw, c)
        logits = logits.reshape(bs, dh*dw, 1)

        logits = logits.repeat(1, 1, c)
        # pos_embed
        # logits += self.pos_embed
        # query += self.pos_embed
        out = self.PromptTransformer(logits, query, query)
        out = out.reshape(bs,dh,dw,c).permute(0,3,1,2)
        out = torch.cat([tran_feat, out],dim=1)
        ...
        #(B, N, Eq) x (B, Eq, N) -->(B, N, N)
        #(B, N, N) x (B, N, E) -> B,N,E
        # out = self.PromptTransformer(
        #     query,
        #     query,
        #     query,
        #     prompt = logits
