...
@NECKS.register_module()
class MultiScaleLSSFPN3D(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 with_cp=False):
    ...
    def forward(self, feats):
        x_8, x_16, x_32 = feats
        x_16_8 = self.up1(x_16)
        x_32_8 = self.up2(x_32)
        x_32_16 = self.up1(x_32)
        x8 = torch.cat([x_8, x_16_8, x_32_8], dim=1)
        x16 = torch.cat([x_16, x_32_16], dim=1)
        x32 = x_32
        if self.with_cp:
            x8 = checkpoint(self.conv1, x8)
            x16 = checkpoint(self.conv2, x16)
            x32 = checkpoint(self.conv3, x32)
        else:
            x8 = self.conv1(x8)
            x16 = self.conv2(x16)
            x32 = self.conv3(x32)
