...
class BEVStereo4D(BEVDepth4D):
    ...
    def prepare_bev_feat(self, img, sensor2keyego, ego2global, intrin,
                         ...
                         k2s_sensor, extra_ref_frame, return_img_feat=False):
        if extra_ref_frame:
    ...
    def extract_img_feat(self,
            ...
            if key_frame or self.with_prev:
                ...
                if key_frame:
                    bev_feat, depth, feat_curr_iv, img_feat = \
                        self.prepare_bev_feat(*inputs_curr, True)
                    ...
                    with torch.no_grad():
                        bev_feat, depth, feat_curr_iv = \
                            self.prepare_bev_feat(*inputs_curr, False)
        ...
        x = self.bev_encoder(bev_feat)
        if return_img_feat:
            return [x], depth_key_frame, img_feats
        else:
            return [x], depth_key_frame
    
    def extract_img_feat_step1(self,
                                img,
                                img_metas,
                                pred_prev=False,
                                sequential=False,
        ...
        return bev_feat_list, depth_key_frame
    
    def extract_img_feat_step2(self, bev_feat_list):
        # no pre process
        bev_feat_list = [self.pre_process_net(bev)[0] for bev in bev_feat_list]
        bev_feat = torch.cat(bev_feat_list, dim=1)
        x = self.bev_encoder(bev_feat)
        return [x]