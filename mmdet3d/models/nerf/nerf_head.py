...
class NerfHead(nn.Module):
    ...
    def rebuilt_depth(self, x, y, depth, depth_mask, cam_id, img_shape=(900,1600)):

        batchsize = x.shape[0]
        depth_map = torch.zeros((batchsize, 6, img_shape[0], img_shape[1])).cuda()

        for bs in range(batchsize):
            mask = depth_mask[bs]
            xx = x[bs][mask].long()
            yy = y[bs][mask].long()
            cam = cam_id[bs][mask].long()
            curr_frame_mask = cam!=-1
            xx = xx[curr_frame_mask]
            yy = yy[curr_frame_mask]
            cam = cam[curr_frame_mask]
            dd = depth[bs][curr_frame_mask]
            depth_map[bs][cam,yy,xx]=dd # advance index
    ...
    def forward(self, density, semantic, rays=None, bda=None, **kwargs):
        ...
        for batch_id in range(rays.shape[0]): 
            ...
            if self.use_depth_sup:
                ...
                render_depth.append(results['render_depth'])
            if self.use_sem_sup:
        ...
        depth = self.rebuilt_depth(x=rays[...,0], y=rays[...,1], depth=render_depth, depth_mask=depth_mask, cam_id=rays[...,13])
        return losses, depth



@HEADS.register_module()
class NerfHeadDense(nn.Module):
    def __init__(self, 
            point_cloud_range,
            voxel_size,
            scene_center=None,
    ...
    def forward(self, density,  rays=None, label_depth=None, bda=None, **kwargs):
        # import pdb;pdb.set_trace()s
        # rays shape(B N H W 13)
        B, N, H, W, _ = rays.shape
        coord = rays[...,0:2]
        gt_depths = rays[..., 2]
        gt_semantics = rays[..., 3]
        ray_o = rays[..., 4:7]
        ...
        for batch_id in range(rays.shape[0]): 
            ...
            # render depth & semantic
            target_depth = label_depth[batch_id].flatten()
            render_depth = self.render_depth(results)
            depth_mask = torch.logical_and(target_depth>0,target_depth<52)
            results['render_depth'] = render_depth[depth_mask]
            results['target_depth'] = target_depth[depth_mask]
        ...
        render_depth = torch.stack(render_depths).reshape(B,N,H,W)
        return losses, render_depth
